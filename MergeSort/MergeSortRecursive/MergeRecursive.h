//
// Created by vincenzo on 19/12/22.
//

#ifndef ASD_MERGERECURSIVE_H
#define ASD_MERGERECURSIVE_H

/*
 *  Progettare ed implementare una procedura Merge
    ricorsiva
 */

class MergeRecursive {
private:
    static void merge(int c[], int a[], int N, int b[], int M);
public:
    static void MergeRec(int a[], int left, int right);
};

void MergeRecursive::merge(int *c, int *a, int N, int *b, int M) {
    if (N+M == 0)
        return;
    if (N == 0) {
        *c = *b;
        merge(c+1, a, N, b+1, M-1);
    }
    else if ( M == 0 ) {
        *c = *a;
        merge(c+1,a+1,N-1,b,M);
    }
    else if ( *a < *b ) {
        *c = *a;
        merge(c+1,a+1,N-1,b,M);
    }
    else {
        *c = *b;
        merge(c+1,a,N,b+1,M-1);
    }
}

void MergeRecursive::MergeRec(int *a, int left, int right) {
    if (left < right) {
        int med = (left+right)/2;
        int tot_elesx = med-left+1;
        int tot_eledx = right-med;
        int dim_out = right-left+1;

        // Alloca il vettore di appoggio
        int *out = new int[dim_out];

        // Fonde le due parti del vettore nel vettore di out
        //out = merge(out, a+left, tot_elem_sx, a+(center+1), tot_elem_dx, 0, 0, 0);
        merge(out,a+left,tot_elesx,a+(med+1),tot_eledx);

        // Ricopia il vettore di appoggio nel vettore originale
        for (int idx=0;idx<right-left+1;idx++)
            a[left+idx]=out[idx];

        delete [] out;
    }
}

#endif //ASD_MERGERECURSIVE_H
